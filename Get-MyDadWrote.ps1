$feedUrl = "https://feeds.acast.com/public/shows/mydadwroteaporno"
$outDir = $Global:PodcastPath + [IO.Path]::DirectorySeparatorChar + "My Dad Wrote A Porno"
...
foreach ($item in $xmlContent.rss.Channel.item) {
    ...
    if ($item.title[0].'#cdata-section'.Length -eq 0) {
        $realTitle = $item.title[0]
    } else {
        $realTitle = $item.title[0].'#cdata-section'
    }

    # This podcast has titles under cdata-section sometimes

    # Replace any spaces at the beginning and end of the title
    # Remove certain characters. Plus remove any S00E00 in the title.
    $episodeTitle = $realTitle -replace '[/:]','' -replace '^\s+','' -replace '\s+$',''
    # Additional substitutions for this podcast 
